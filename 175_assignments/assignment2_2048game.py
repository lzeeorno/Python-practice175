...
class Grid():
    ...
    def getCell(self, cell):
        ...
        12 13 14 15
        """
        row = cell // self.col #how we calculate the cell in which row
        col = cell % self.col  #how ew calculate the cell in which column
    ...
    def collapseRight(self):
        ...
        row_is_collapsed = False
        for row in range(self.row):
            # get row to the right 
            row_list = []
            for col in range(self.col):
                cell = self.getCell(row * self.col + col)
                row_list.append(cell)
            row_list.reverse() # reverse the row list, b/c it it read from right to left

            # collapse this row to the right
            row_collapsed, is_collapsed = self.collapseRow(row_list)
            if is_collapsed:
                row_is_collapsed = True
            row_collapsed.reverse() # reverse 
